...
class AudioPreprocessor(object):
    ...
    def compute_mfccs(self, data):
     #   print('input data.shape----', data.shape)
        ...
        win_length = 250  # self.n_fft
        fft_window = signal.get_window(window='hann', Nx=win_length, fftbins=True)
        # Pad the window out to n_fft size
        fft_window = librosa.util.pad_center(fft_window, self.n_fft)

        # Reshape so that the window can be broadcast
        fft_window = fft_window.reshape((-1, 1))
        fft_window = np.array(1.0 + 0.0 * fft_window)
        # Pad the time series so that frames are centered
        if center:
            data = np.pad(data, int(self.n_fft // 2), mode=pad_mode)
        # Window the time series.
        data = np.int16(data * 32768)

        y_frames = librosa.util.frame(data, frame_length=win_length, hop_length=self.hop_length)
        numframes = 32
        y_frames = y_frames[:, 1:numframes + 1]
        fft_data_pad = (librosa.util.fix_length(y_frames.T, self.n_fft)).T

        # Pre-allocate the STFT matrix
        stft_matrix = np.empty((int(1 + self.n_fft // 2), fft_data_pad.shape[1]),
                               dtype=dtype,
                               order='F')
        ...
        # print('fft_window:', fft_window.shape)
        for bl_s in range(0, stft_matrix.shape[1], n_columns):
            bl_t = min(bl_s + n_columns, stft_matrix.shape[1])
            frame_window = fft_window * fft_data_pad[:, bl_s:bl_t]
        # FFT#####################################################################################
        tot_layer = 8
        n_fft = self.n_fft
        nfft = self.n_fft
        ...
        for i in range(0, numframes):
            ...
            fft_frame = fft_frame-np.floor(0.96875*fft_frame_delay[0:nfft])
        ...
        weights = np.zeros((self.n_mels, int(1 + self.n_fft // 2)), dtype=np.float64)

        # Center freqs of each FFT bin
        fftfreqs = librosa.fft_frequencies(sr=self.sr, n_fft=self.n_fft)

        # 'Center freqs' of mel bands - uniformly spaced between limits
        mel_f = librosa.mel_frequencies(self.n_mels + 2, fmin=self.f_min, fmax=self.f_max, htk=htk)
        ...
        if norm in (1, 'slaney'):
            ...
            enorm = 2.0 / (mel_f[2:self.n_mels + 2] - mel_f[:self.n_mels])
        ...
        #data_2a_compare = 1 * data_2a
        #data_2a_compare[data_2a_compare > 0] = np.round((np.log(data_2a_compare[data_2a_compare > 0]) - np.log(
        #    32768)) * log_int_quant) / log_int_quant  # compare with data_2
        #data_2a[data_2a > 0] = np.round((np.log(data_2a[data_2a > 0])) * log_int_quant)  # fpga implementation
        #data = data_2a  # _compare
        mfsc_bit = 16
        inter_num = 32
        log_num = 64
        dec_bit = 8
        ...
        for t in range(NUMFRAMES):
            delta_feat[:, t] = np.floor(np.dot(padded[:, t: t + 2 * N + 1], np.arange(-N,N + 1)) * 149 / 2 ** 12) + 2 ** 15

        delta_delta_feat = np.empty_like(feat)
        delta_padded = np.pad(delta_feat, ((0, 0), (N, N)), mode='edge')  # padded version of feat
        for t in range(NUMFRAMES):
            delta_delta_feat[:, t] = np.floor(np.dot(delta_padded[:, t: t + 2 * N + 1], np.arange(-N,N + 1)) * 149 / 2 ** 12) + 2 ** 15

        mfsc_data = np.zeros([3, n_mels, NUMFRAMES])
        mfsc_data[0, :, :] = np.round(feat / 256)
        mfsc_data[1, :, :] = np.round(delta_feat / 256)
        mfsc_data[2, :, :] = np.round(delta_delta_feat / 256)
