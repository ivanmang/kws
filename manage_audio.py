...
class AudioPreprocessor(object):
    ...
    def compute_mfccs(self, data):
        data = librosa.feature.melspectrogram(
            ...
            fmax=self.f_max)
        # tmp=data.copy()
        # if self.d_max < np.amax(data):
        #     self.d_max = np.amax(data)
        #     print('new max {:}'.format(np.amax(data)))
        #     # quantization for mfsc data
        # data[data > self.d_min_value] = np.round(data[data > self.d_min_value] * self.value) / self.value
        #
        # # minimum value clamp
        # data[data <= self.d_min_value] = self.d_min_value
        # data[data > self.d_min_value] = np.log(data[data > self.d_min_value])
        #
        # # data = [np.matmul(self.dct_filters, x) for x in np.split(data, data.shape[1], axis=1)]
        # data = np.array(data, order="F").astype(np.float32)
        # # quantization for log mfsc data
        # if self.log_d_max < np.amax(data):
        #     self.log_d_max = np.amax(data)
        #     # self.scale = self.d_max / 256
        #     self.inv_log_scale = round(255 / self.log_d_max * self.log_bits_value) / self.log_bits_value
        #     print('new data max: {:}, inv_scale: {:} '.format(self.log_d_max, self.inv_log_scale))
        #
        # # d_max = max(data.max(), -data.min())
        # # scale = d_max / 256
        # d_int = np.round(data * self.inv_log_scale)
      ...
      #  data = [np.matmul(self.dct_filters, x) for x in np.split(data, data.shape[1], axis=1)]
        ...
        if self.d_max < np.amax(data):
            self.d_max = np.amax(data)
            print('new max {:}'.format(np.amax(data)))
        if self.d_min> np.amin(data):
            self.d_min = np.amin(data)
            print('new min {:}'.format(np.amin(data)))

     #   print('data.shape----', data.shape)
        delta_1 = self.delta(data, N=8)
        delta_2 = self.delta(delta_1, N=8)
        data = self.quantize_tensor(data, num_bits=8)
        delta_1 = self.quantize_tensor(delta_1, num_bits=8)
        delta_2 = self.quantize_tensor(delta_2, num_bits=8)

        data = np.array([(i - 128) for i in data])
        delta_1 = np.array([(i - 128) for i in delta_1])
        delta_2 = np.array([(i - 128) for i in delta_2])

        return data, delta_1, delta_2
