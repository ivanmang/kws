...
class SpeechDataset(data.Dataset):
    ...
    def default_config():
        config = {"group_speakers_by_id": True, "silence_prob": 0.1, "noise_prob": 0.8, "n_dct_filters": 32,
                  "input_length": 8000, "n_mels": 32, "timeshift_ms": 100, "unknown_prob": 0.2, "train_pct": 80,
                  "dev_pct": 10, "test_pct": 10,
    ...
    def collate_fn(self, _data):
        ...
        for audio_data, label in _data:
            ...
            if self.audio_preprocess_type == "MFCCs_delta":
                mfsc_data=torch.Tensor(self.audio_processor.compute_mfccs(audio_data)).unsqueeze(0)
                x = mfsc_data if x is None else torch.cat((x, torch.Tensor(mfsc_data)), 0)
            ...
            if self.audio_preprocess_type == "MFCCs":
                ...
                audio_tensor = torch.from_numpy(self.audio_processor.compute_mfccs(np.array(audio_data)).reshape(1, -1, 32))
